...
graph TD
    ...
    B --> C["VStack / HStack 등<br/>내장 컨테이너"]
    B --> D["커스텀 컨테이너 뷰"]
    D --> E["단일 슬롯<br/>CardView<Content>"]
    D --> F["멀티 슬롯<br/>SectionContainer<H,C,F>"]
...
```mermaid
flowchart LR
    A["클로저 안에<br/>뷰 나열"] --> B{"컴파일러 분석"}
    B -->|"뷰 나열"| C["buildBlock(_:_:...)<br/>-> TupleView"]
    B -->|"if-else"| D["buildEither(first:/second:)<br/>-> _ConditionalContent"]
    B -->|"if (단독)"| E["buildOptional(_:)<br/>-> Optional<View>"]
    B -->|"for-in"| F["buildArray(_:)<br/>-> [View]"]
    C --> G["하나의 View 타입 반환"]
    D --> G
    E --> G
    F --> G
...
flowchart TD
    ...
    D -.->|"Footer == EmptyView"| H["편의 이니셜라이저<br/>(footer 생략 가능)"]
```
