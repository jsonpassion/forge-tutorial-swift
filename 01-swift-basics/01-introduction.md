...
참고 이미지: [Xcode 공식 아이콘 미리보기](https://upload.wikimedia.org/wikipedia/commons/thumb/b/b9/Xcode_Mojave_icon.png/240px-Xcode_Mojave_icon.png) · [Apple Park 캠퍼스](https://picsum.photos/id/160/1200/800.jpg)

> 📊 **그림 1**: Xcode가 제공하는 개발 워크플로우 전체 흐름

```mermaid
flowchart LR
    A["코드 작성<br/>(Editor)"] --> B["빌드<br/>(Compiler)"]
    B --> C["시뮬레이터<br/>테스트"]
    C --> D["디버깅<br/>(Instruments)"]
    D --> A
    C --> E["앱스토어<br/>배포"]
```
...
> 📊 **그림 2**: Xcode 인터페이스 4대 영역 배치

```mermaid
graph LR
    subgraph Xcode
        direction LR
        N["📁 Navigator<br/>(왼쪽)<br/>⌘0"]
        subgraph 중앙영역
            direction LR
            E["✏️ Editor<br/>코드 작성"]
            C["👁️ Canvas/Preview<br/>실시간 미리보기<br/>⌥⌘↩"]
        end
        I["🔧 Inspector<br/>(오른쪽)<br/>⌥⌘0"]
        N --> E
        E --> C
        C --> I
    end
...
Swift는 2010년 7월, Apple의 엔지니어 **Chris Lattner**가 자신의 노트북에서 혼자 시작한 프로젝트였습니다. 당시 코드명은 **"Shiny"** — 말 그대로 "반짝이는 새 것"이라는 뜻이었는데, TV 드라마 Firefly의 레퍼런스이기도 했다고 해요. [Swift 언어 공식 로고 보기](https://upload.wikimedia.org/wikipedia/commons/9/9d/Swift_logo.svg)
