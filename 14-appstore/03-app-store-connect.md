...
> 📊 **그림 1**: App Store Connect 전체 워크플로우

```mermaid
flowchart LR
    A["앱 등록"] --> B["메타데이터 작성"]
    B --> C["가격/지역 설정"]
    C --> D["Privacy Labels 작성"]
    D --> E["빌드 업로드"]
    E --> F["TestFlight 베타 테스트"]
    F --> G["심사 제출"]
    G --> H{"심사 결과"}
    H -->|승인| I["App Store 출시"]
    H -->|리젝| J["수정 후 재제출"]
...
> 📊 **그림 3**: Privacy Nutrition Labels 선언 구조

```mermaid
graph TD
    A["Privacy Labels 작성"] --> B["수집 데이터 유형"]
    A --> C["데이터 사용 목적"]
    B --> D["연락처 정보"]
    B --> E["위치 데이터"]
    B --> F["식별자"]
...
> 📊 **그림 2**: TestFlight 내부/외부 테스트 흐름

```mermaid
flowchart TD
    A["Xcode Archive"] --> B["App Store Connect<br/>빌드 업로드"]
    B --> C["빌드 처리 완료"]
    C --> D["내부 테스트"]
    C --> E["외부 테스트"]
    D --> F["팀 멤버 최대 100명<br/>심사 불필요"]
    E --> G["Beta App Review"]
    G --> H["외부 사용자 최대 10,000명"]
    F --> I["피드백 수집"]
    H --> I
    I --> J["버그 수정 & 개선"]
    J --> B
