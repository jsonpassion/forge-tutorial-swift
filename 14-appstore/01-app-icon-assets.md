...
> 📊 **그림 1**: Icon Composer의 레이어 구조와 렌더링 흐름

```mermaid
flowchart TD
    A["배경 레이어(Back)"] --> D["Icon Composer"]
    B["전경 레이어(Front)"] --> D
    C["추가 레이어(선택)"] --> D
    D --> E{"Glass Effect?"}
    E -->|적용| F["Liquid Glass 아이콘"]
    E -->|미적용| G["일반 아이콘"]
    F --> H["라이트 모드"]
    F --> I["다크 모드"]
    F --> J["틴트 모드"]
...
> 📊 **그림 2**: 스크린샷 구성 전략 — Value → Usage → Trust

```mermaid
flowchart LR
    subgraph 첫노출["검색 결과 노출 영역"]
        A["1~2장<br/>가치 제안"] --> B["3~5장<br/>사용 시나리오"]
    end
    B --> C["6~8장<br/>신뢰 구축"]
    C --> D["9~10장<br/>추가 기능"]
    A -.->|"첫 3장이 승부처"| B
    style 첫노출 fill:#e8f4fd,stroke:#4a90d9
```
...
> 📊 **그림 3**: App Store 제품 페이지 시각 에셋 구성

```mermaid
flowchart TD
    A["App Store 제품 페이지"] --> B["앱 아이콘<br/>1024×1024"]
    A --> C["스크린샷<br/>최대 10장"]
    A --> D["앱 미리보기<br/>최대 3개"]
    C --> E["iPhone 6.9인치<br/>1320×2868"]
    C --> F["iPhone 6.3인치<br/>1206×2622"]
    C --> G["iPad 13인치<br/>2064×2752"]
    D --> H["15~30초"]
    D --> I["앱 내 화면 녹화"]
    D --> J["자동 재생"]
```
