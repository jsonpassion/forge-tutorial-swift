...
> 📊 **그림 2**: Glass 타입과 수정자 조합 선택 흐름

```mermaid
flowchart TD
    A["유리 효과 적용"] --> B{"배경이 미디어 중심?"}
    B -->|"예"| C[".clear"]
    B -->|"아니오"| D[".regular"]
    C --> E{"색상 의미 전달 필요?"}
    D --> E
...
> 📊 **그림 3**: GlassEffectContainer의 블렌딩 동작

```mermaid
flowchart LR
    subgraph WITHOUT["컨테이너 없이"]
        A1["🏠 홈"] ~~~ A2["🔍 검색"] ~~~ A3["⚙️ 설정"]
    end
    subgraph WITH["GlassEffectContainer 안에"]
        B1["🏠 홈 + 🔍 검색 + ⚙️ 설정"]
    end
    WITHOUT -->|"GlassEffectContainer로 감싸기"| WITH
    style WITHOUT fill:#ffffff11,stroke:#aaa
    style WITH fill:#4a9eff22,stroke:#4a9eff
```
...
> 📊 **그림 1**: Liquid Glass의 레이어 구분 — 콘텐츠 vs 네비게이션

```mermaid
graph TD
    subgraph NAV["네비게이션 레이어 (Liquid Glass 적용)"]
        N1["NavigationBar"]
        N2["TabBar"]
        N3["Toolbar"]
        N4["플로팅 버튼"]
    end
    subgraph CONTENT["콘텐츠 레이어 (유리 효과 X)"]
        C1["리스트 / 텍스트"]
        C2["이미지 / 미디어"]
        C3["입력 폼"]
    end
    NAV -->|"위에 떠 있음"| CONTENT
    style NAV fill:#4a9eff22,stroke:#4a9eff
    style CONTENT fill:#66666622,stroke:#999
```
