...
> 📊 **그림 1**: Swift 프로퍼티 종류 전체 구조

```mermaid
graph TD
    A["프로퍼티"] --> B["인스턴스 프로퍼티"]
    A --> C["타입 프로퍼티 (static)"]
    B --> D["저장 프로퍼티<br/>Stored"]
    B --> E["연산 프로퍼티<br/>Computed"]
    D --> F["let 상수"]
    D --> G["var 변수"]
    G --> H["프로퍼티 감시자<br/>willSet / didSet"]
    G --> I["lazy 지연 초기화"]
    E --> J["get 전용"]
    E --> K["get + set"]
```
...
sequenceDiagram
    ...
    participant P as 프로퍼티
    participant W as willSet
    participant D as didSet
    C->>P: 새 값 할당
    P->>W: willSet(newValue) 호출
    Note over W: 아직 이전 값 유지
    W-->>P: 완료
    P->>P: 값 실제 변경
    P->>D: didSet(oldValue) 호출
    Note over D: 새 값으로 변경됨
    D-->>P: 완료
    P-->>C: 할당 완료
```
...
> 📊 **그림 3**: 인스턴스 프로퍼티 vs 타입 프로퍼티

```mermaid
flowchart LR
    subgraph TYPE["AppConfig 타입"]
        S1["static appName<br/>= 'Swift 마스터'"]
        S2["static launchCount<br/>= 0"]
    end
    subgraph I1["인스턴스 A"]
        P1["name = '설정1'"]
    end
