...
```mermaid
graph LR
    E["Employee<br/>(참조 카운트: 1)"] -- "strong: team" --> T["Team<br/>(참조 카운트: 1)"]
    T -- "strong: leader" --> E
...
```mermaid
graph LR
    E["Employee<br/>(참조 카운트: 1)"] -- "strong: team" --> T["Team<br/>(참조 카운트: 2)"]
    T -. "weak: leader" .-> E
    V1["변수 employee"] --> E
    V2["변수 team"] --> T
...
```mermaid
flowchart LR
    A["앱 실행"] --> B["화면 진입<br/>(push)"]
    B --> C["화면 이탈<br/>(pop)"]
    C --> D["Memory Graph<br/>캡처"]
    D --> E{"해제 안 된<br/>객체 있음?"}
    E -- "없음" --> F["정상 O"]
    E -- "있음" --> G["참조 그래프<br/>확인"]
    G --> H{"순환 참조?"}
    H -- "예" --> I["weak/unowned<br/>적용"]
    H -- "아니오" --> J["다른 원인<br/>조사"]
```
