...
```mermaid
flowchart LR
    A["Xcode에서<br/>⌘I 실행"] --> B["Release 빌드"]
    B --> C["템플릿 선택"]
    C --> D["Record 시작"]
    D --> E["앱 조작<br/>(시나리오 재현)"]
    E --> F["Stop & 분석"]
...
flowchart LR
    subgraph PRE["Pre-main 단계"]
        A["dyld 로딩"] --> B["동적 프레임워크<br/>링킹"]
        B --> C["ObjC 런타임<br/>초기화"]
        C --> D["static initializer<br/>실행"]
    end
    subgraph POST["Post-main 단계"]
        E["AppDelegate<br/>초기화"] --> F["UI 구성"]
        F --> G["첫 화면<br/>렌더링"]
    end
    D --> E
    G --> H["O 사용자에게<br/>화면 표시"]
