...
> 📊 **그림 1**: Actor 격리 도메인과 Sendable 전달

```mermaid
flowchart LR
    subgraph 도메인A["Actor A 격리 도메인"]
        A1["가변 상태"]
    end
    subgraph 도메인B["Actor B 격리 도메인"]
        B1["가변 상태"]
    end
...
> 📊 **그림 3**: Swift 6.0 vs 6.2 동시성 모델 비교

```mermaid
flowchart TD
    subgraph V60["Swift 6.0 모델"]
        direction TB
        A1["모든 코드"] --> A2{"격리 지정?"}
