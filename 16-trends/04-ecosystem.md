...
> 📊 **그림 2**: 서버 사이드 Swift — 프론트/백엔드 모델 공유 구조
...
> 📊 **그림 4**: Swift Evolution 프로포절 과정

```mermaid
stateDiagram-v2
    [*] --> Pitch: 아이디어 제안
    Pitch --> Proposal: 정식 제안서 작성
    Proposal --> Review: 커뮤니티 공개 토론
    Review --> Accepted: 채택
    Review --> Returned: 수정 요청
    Review --> Rejected: 거절
    Returned --> Proposal: 수정 후 재제출
    Accepted --> Implementation: 구현
    Implementation --> Released: Swift 릴리스에 포함
    Released --> [*]
```
