...
```mermaid
flowchart LR
    A["Window\n2D 평면 창"] -->|"깊이 추가"| B["Volume\n3D 고정 공간"]
    B -->|"공간 확장"| C["Immersive Space\n전체 공간 활용"]
    C --- D["Mixed\n현실+가상 공존"]
    C --- E["Progressive\n부분 가상"]
    C --- F["Full\n완전 가상"]
```
...
```mermaid
graph TD
    E["Entity\n빈 컨테이너"] --> MC["ModelComponent\n3D 모양"]
    E --> TC["Transform\n위치/회전/크기"]
    E --> PC["PhysicsBodyComponent\n물리 법칙"]
    E --> CC["CollisionComponent\n충돌 감지"]
    S["System\n매 프레임 실행"] -->|"Component 읽기/쓰기"| E
    S --> PS["물리 시뮬레이션"]
    S --> AS["애니메이션 업데이트"]
```
...
```mermaid
flowchart TD
    A["기존 SwiftUI 앱"] --> B["1단계: Destination 추가\nApple Vision 타겟"]
    B --> C["2단계: 호환성 수정\n#if !os(visionOS)"]
    C --> D["3단계: UI 검증\n유리 배경 대비 확인"]
    D --> E["iPad 호환 모드\n2D 창으로 실행"]
    D --> F["4단계: 공간 요소 추가"]
