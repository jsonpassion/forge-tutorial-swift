...
> 📊 **그림 1**: SwiftUI 고급 애니메이션 도구 전체 구조

```mermaid
flowchart TD
    A["SwiftUI 고급 애니메이션"] --> B["Spring Animation"]
    A --> C["KeyframeAnimator"]
    A --> D["PhaseAnimator"]
    A --> E["contentTransition"]
    B --> B1["프리셋: smooth / snappy / bouncy"]
    B --> B2["커스텀: duration + bounce"]
    C --> C1["시간 기반 제어"]
    C --> C2["프로퍼티별 독립 트랙"]
    D --> D1["상태 기반 순환"]
    D --> D2["자동 반복 / 트리거 모드"]
...
> 📊 **그림 2**: KeyframeAnimator의 다중 트랙 타임라인 구조

```mermaid
sequenceDiagram
    participant T as trigger 이벤트
    participant Y as yOffset 트랙
    participant S as scale 트랙
    participant R as rotation 트랙
    T->>Y: 애니메이션 시작
    T->>S: 애니메이션 시작
    T->>R: 애니메이션 시작
...
> 📊 **그림 4**: KeyframeAnimator vs PhaseAnimator 선택 가이드

```mermaid
flowchart TD
    Q{"어떤 애니메이션이 필요한가?"}
    Q -->|"프로퍼티별 정밀 타이밍 제어"| KF["KeyframeAnimator"]
    Q -->|"단계별 상태 순환/반복"| PA["PhaseAnimator"]
    Q -->|"텍스트/심볼 전환"| CT["contentTransition"]
    KF --> KF1["시간 기반"]
    KF --> KF2["다중 트랙 독립 제어"]
    KF --> KF3["복잡한 모션 시퀀스"]
    PA --> PA1["상태(Phase) 기반"]
    PA --> PA2["자동 반복 또는 트리거"]
    PA --> PA3["로딩, 펄스, 주의 효과"]
    CT --> CT1["numericText: 숫자 롤링"]
    CT --> CT2["symbolEffect: SF Symbol 전환"]
```
