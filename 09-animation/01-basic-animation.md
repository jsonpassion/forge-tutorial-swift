...
flowchart LR
    subgraph 암시적["암시적 애니메이션"]
        A1["상태 변경"] --> A2[".animation 수정자가<br/>변화 감지"]
        A2 --> A3["자동 애니메이션 실행"]
    end
...
flowchart TD
    ...
    A --> C["스프링 기반"]
    B --> D[".linear<br/>일정 속도"]
    B --> E[".easeIn<br/>느리게->빠르게"]
    B --> F[".easeOut<br/>빠르게->느리게"]
    B --> G[".easeInOut<br/>느리게->빠르게->느리게"]
    C --> H[".spring<br/>바운스 조절 가능"]
    C --> I[".bouncy<br/>큰 바운스"]
    C --> J[".smooth<br/>바운스 없음"]
    C --> K[".snappy<br/>빠르고 약간 바운스"]
```
