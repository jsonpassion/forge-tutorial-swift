...
```mermaid
flowchart TD
    A["'@Model'<br/>영속 모델 정의"] -->|"저장 대상"| B["ModelContainer<br/>데이터베이스 관리"]
    B -->|"작업 공간 제공"| C["ModelContext<br/>CRUD 수행"]
...
flowchart LR
    ...
    B --> C["PersistentModel<br/>프로토콜 채택"]
    B --> D["Observable<br/>프로토콜 채택"]
    B --> E["변경 추적<br/>코드 생성"]
    B --> F["DB 스키마<br/>자동 생성"]
    C --> G["SwiftData가<br/>관리 가능"]
    D --> H["SwiftUI 뷰<br/>자동 업데이트"]
```
